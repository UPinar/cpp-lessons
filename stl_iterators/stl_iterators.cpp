...
/*
  // when reallocation occurs, iterators and references
  // will become invalid.
  ...
  int main()
  {
    ...
    // when a new element will be added reallocation will occur.
...
/*
  // iterators and references after the insertion point 
  // will become invalid.
...
/*
  // iterators and references after the deletion point 
  // will become invalid.
...
/*
  ...
  // if string's length == 5 remove
  // if string's length == 6, add 1 more.

  int main()
  {
    std::vector<std::string> svec { 
      "metin", "hasan", "mert", "necati", "remzi", 
      "turhan", "ali", "veli", "selami" };
    ...
    for (auto iter = svec.begin(); iter != svec.end(); ++iter) 
    {
      ...
      {
        svec.erase(iter);
        // when an elemenent will be erased,
        // "iter" will become invalid
        // increasing invalid iterator  
        // will cause undefined behaviour(UB).
      }
      else if (iter->length() == 6)
      {
        svec.insert(iter, *iter);
        // if reallocation occurs, iterators will become invalid.
        // increasing invalid iterator 
        // will cause undefined behaviour(UB).
      }
    }
  ...
  int main()
  {
    ...
    while (iter != svec.end()) 
    {
      if (iter->length() == 5) 
      {
        ...
        // "erase" member function will return an iterator
        // to the next element of the erased element.
      }
      else if (iter->length() == 6) 
      {
        iter = svec.insert(iter, *iter);
        // "insert" member function will return 
        // an iterator to the inserted elements position.

        std::advance(iter, 2);
        // aa bb cccccc dd eeee 
        // aa bb cccccc cccccc dd eeee
        //       | returns an iterator to this position
        // so need to increase "iter"'s position by 2.
      }
      else
        ++iter;
    }
...
/*
  ...
  private:
    const char* mp_seperator;
    std::ostream& m_ostream;

  public:
    Ostream_Iterator(std::ostream& os, const char* p_sep = "") 
      : m_ostream(os), mp_seperator(p_sep) {}
    ...
    {
      m_ostream << val << mp_seperator;
      return *this;
    }

    // postfix operator++, prefix operator++ and operator*
    // are neutral operators. They do nothing.
  };
  ...
  int main()
  {
    ...
    Copy( svec.begin(), svec.end(), 
          Ostream_Iterator<std::string>{ std::cout, " " });
    // console output -> hello world we are live from izmir
    ...
    Copy( svec.begin(), svec.end(), 
          Ostream_Iterator<std::string>{ ofs, "\n" });
...
/*
  ...
  public:
    ...
    {
      std::cout << "Myclass& operator=(Myclass&&)\n";
      return *this;
    }
  };

  template <typename Iter>
  std::move_iterator<Iter> Make_Move_Iterator(Iter iter)
  {
  ...
  int main()
  {
    ...
    std::string str1 = *iter;   // copy constructor

    std::cout << "str_vec[0].length() = " 
              << str_vec[0].length() << '\n';
    // output -> str_vec[0].length() = 5 

    // becuase of copy constructor is called 
    // str_vec[0] will be copied to str1.

    // -------------------------------------------------

    std::string str2 = std::move(*iter); // move constructor

    std::cout << "str_vec[0].length() = " 
              << str_vec[0].length() << '\n';
    // output -> length = 0

    // because of move constructor is called
    // str_vec[0] will be moved to str2.

    // -------------------------------------------------

    *iter = "hello";  // copy assignment

    for (auto str : str_vec)
      std::cout << str << ' ';
    // output -> hello galaxy universe
    std::cout << '\n';
    ...
    move_iter_t move_iter{ str_vec.begin() };
    std::string str3 = *move_iter;    // move constructor
    ...
    // because of move constructor is called
    // when std::move_iterator is dereferenced
    // str_vec[0] will be moved to str3.

    // -------------------------------------------------

    using move_iter_myclass_t = 
        std::move_iterator<std::vector<Myclass>::iterator>;

    std::vector<Myclass> mx_vec(100);

    move_iter_myclass_t move_iter_vec_beg{ mx_vec.begin() };
    auto move_iter_vec_end = Make_Move_Iterator(mx_vec.end());

    // both "move_iter_vec_beg" and "move_iter_vec_end"'s 
    // type is `std::move_iterator<std::vector<Myclass>::iterator>`

    Myclass m1 = *move_iter_vec_beg;
    // output -> Myclass(Myclass&&)

    // -----------------------------------------------------

    Myclass m2 = *move_iter_vec_end;    
    // output -> Myclass(Myclass&&)
    Myclass m3 = *make_move_iterator(mx_vec.begin());	
    // output -> Myclass(Myclass&&)
...
/*
  ...
  int main()
  {
    ...
    std::vector<Myclass> vec_1(3);
    std::vector<Myclass> vec_2(3);

    // -----------------------------------------------------

    std::copy(vec_1.begin(), vec_1.end(),     // source
              vec_2.begin());                 // destination
    ...
    std::copy(std::make_move_iterator(vec_1.begin()), // src_beg
              std::make_move_iterator(vec_1.end()),   // src_end
              vec_2.begin());                         // dest
    ...
    // std::move algorithm in <algorithm> header

    std::move(vec_1.begin(), vec_1.end(), vec_2.begin());
    // output ->
    //  Myclass& operator=(Myclass&&)
    //  Myclass& operator=(Myclass&&)
    //  Myclass& operator=(Myclass&&)
