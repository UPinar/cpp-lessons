...
/*
  ...
  lower bound(key) :
  position of the first element that is equal or greater than key.
  ...
  upper bound(key) :
  position of the first element that is gretaer than key.
  ...
  difference(distance) between lower bound and upper bound
  is equal to how many key values exists in the container.
...
/*
  ...
  int main()
  {
    std::multiset<int> i_mset { 
      2, 5, 7, 7, 7, 7, 9, 9, 9, 13, 13, 45, 89 
    };
    ...
    auto l_bound_iter = i_mset.lower_bound(7);
    std::cout << "index of lower_bound = " 
              << std::distance(i_mset.begin(), l_bound_iter) << '\n';
    // output -> index of lower_bound = 2

    auto u_bound_iter = i_mset.upper_bound(7);
    std::cout << "index of upper_bound = " 
              << std::distance(i_mset.begin(), u_bound_iter) << '\n';
    // output -> index of upper bound = 6

    std::cout << "distance for equal range = " 
              << std::distance(l_bound_iter, u_bound_iter) << '\n';
    // output -> distance for equal range = 4

    // there are 4 elements that has value 7 in the container.
    ...
    l_bound_iter = i_mset.lower_bound(6);
    std::cout << "index of lower_bound = " 
              << std::distance(i_mset.begin(), l_bound_iter) << '\n';
    // output -> index of lower_bound = 2

    u_bound_iter = i_mset.upper_bound(6);
    std::cout << "index of upper_bound = " 
              << std::distance(i_mset.begin(), u_bound_iter) << '\n';
    ...
    // there are 0 elements that has value 6 in the container.
...
/*
  ...
  int main()
  {
    ...
    // key = 11
    // 3, 7, 8, 11, 34, 56, 78, 90
    //          L   U

    auto l_bound_index = 
      std::lower_bound(ivec.begin(), ivec.end(), 11) - ivec.begin();

    auto u_bound_index = 
      std::upper_bound(ivec.begin(), ivec.end(), 11) - ivec.begin();

    std::cout << l_bound_index << '\n';   // output -> 3
    std::cout << u_bound_index << '\n';   // output -> 4

    // because of std::vector's iterator type is 
    // std::random_access_iterator 
    // substraction in iterators is a valid operation.
  }
*/

/*
  // adding random values to std::vector in a sorted order.
  ...
  int main()
  {
    ...
    for (int i = 0; i < 5; ++i)
    {
      auto name = rname() + ' ' + rfname();

      auto lower_bound_iter = 
        std::lower_bound(svec.begin(), svec.end(), name);

      svec.insert(lower_bound_iter, name);
      print(svec, "\n");
    }
...
/*
  ...
  int main()
  {
    ...
    pair_type r_p = std::equal_range(svec.begin(), svec.end(), town);

    std::cout << "how many " << town << " = " 
              << std::distance(r_p.first, r_p.second) << '\n';

    std::cout << "first index = " 
              << std::distance(svec.begin(), r_p.first)
              << ", last index = " 
              << std::distance(svec.begin(), r_p.second) << '\n';

    // output ->
    //  how many ankara = 13
    //  first index = 87, last index = 100
    ...
    auto [iter_lower, iter_upper] = 
      std::equal_range(svec.begin(), svec.end(), town);

    std::cout << "how many " << town << " = " 
              << std::distance(iter_lower, iter_upper) << '\n';

    std::cout << "first index = " 
              << std::distance(svec.begin(), iter_lower)
              << ", last index =  " 
              << std::distance(svec.begin(), iter_upper) << '\n';
...
/*
  ...
  int main()
  {
    ...
    for (int i = 0; i < 10; ++i)
    {
      auto name = rname() + ' ' + rfname();
      auto lower_bound_iter = 
        std::lower_bound(svec.begin(), svec.end(), name, &str_pred);
      svec.insert(lower_bound_iter, name);
    }
    print(svec, "\n");
    // output ->
...
/*
  ...
  int main()
  {
    ...
    for (int i = 0; i < 5; ++i) 
    {
      std::string town = rtown();
      auto iter = std::lower_bound(svec.begin(), svec.end(), town);
    ...
    //  aksaray karabuk
    //  --------------------------------------
    //  aksaray karabuk ordu
    //  --------------------------------------
    //  aksaray giresun karabuk ordu
    //  --------------------------------------
    //  aksaray elazig giresun karabuk ordu
    //  --------------------------------------
  }
*/
...
/*
  ...
  int main()
  {
    ...
    std::string searched_str{"ddd"};

    auto [iter_lower, iter_upper] = 
      std::equal_range(str_vec.begin(), str_vec.end(), searched_str);

    std::cout << "lower_bound idx = " 
              << std::distance(str_vec.begin(), iter_lower) << '\n';
    // output -> lower_bound idx = 3

    std::cout << "upper_bound idx = " 
              << std::distance(str_vec.begin(), iter_upper) << '\n';
    // output -> upper_bound idx = 6

    std::printf("count of '%s' in str_vec = %d \n", 
                searched_str.c_str(), 
                std::distance(iter_lower, iter_upper));
    // output -> count of 'ddd' in str_vec = 3
  }
*/
...
/*
  ...
  #include <list>
  #include <algorithm>  
  // std::sort, std::set_intersection, std::set_union, 
  // std::set_difference, std::set_symmetric_difference
  ...
  int main()
  {
    ...
    rfill(ivec, 15, Irand{ 0, 50 });
    rfill(ilist, 15, Irand{ 0, 50 });

    std::sort(ivec.begin(), ivec.end());
    ilist.sort();

    print(ivec);    
    // output -> 0 3 5 6 7 10 12 16 27 27 33 35 40 41 44
    print(ilist);
    // output -> 4 10 12 18 29 30 31 33 37 37 38 39 43 47 48

    // -----------------------------------------------------

    std::set_intersection(
        ivec.begin(), ivec.end(),                       // src_1
        ilist.begin(), ilist.end(),                     // src_2
        std::ostream_iterator<int>{ std::cout, " " });  // dest
    // output -> 10 12 33
    ...
    std::set_union(ivec.begin(), ivec.end(), 
                  ilist.begin(), ilist.end(), 
                  std::ostream_iterator<int>{ std::cout, " " });

    // output ->
    //  0 3 4 5 6 7 10 12 16 18 27 27 29 30 31 33 35 
    //  37 37 38 39 40 41 43 44 47 48

    std::cout << '\n';

    // -----------------------------------------------------

    // std::vector have but std::list does not have
    std::set_difference(ivec.begin(), ivec.end(), 
                        ilist.begin(), ilist.end(), 
                        std::ostream_iterator<int>{ std::cout, " "});
    // output -> 0 3 5 6 7 16 27 27 35 40 41 44

    std::cout << '\n';

    // std::list have but std::vector does not have
    std::set_difference(ilist.begin(), ilist.end(), 
                        ivec.begin(), ivec.end(), 
                        std::ostream_iterator<int>{ std::cout, " "});
    // output -> 4 18 29 30 31 37 37 38 39 43 47 48

    std::cout << '\n';

    // -----------------------------------------------------

    // (std::vector and std::list have seperately) - 
    // (std::vector and std::list both have)
    ...
    std::set_symmetric_difference(
        ...
        std::ostream_iterator<int>{ std::cout, " " });
    // output -> 
    //  0 3 4 5 6 7 16 18 27 27 29 30 31 35 37 37 
...
/*
  ...
  int main()
  {
    ...
    if (std::binary_search(ivec.begin(), ivec.end(), ival))
      std::cout << ival << " is exists in vector\n";
    else
      std::cout << ival << " is NOT exists in vector\n";
    // output -> 45 is NOT exists in vector

    // ----------------------------------------------------

    if (std::binary_search( 
          ivec.begin(), ivec.end(), ival, 
